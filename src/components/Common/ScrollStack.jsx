...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // 🚀 PERFORMANCE FIX: Batch all DOM reads (layout calcs) before ANY DOM writes to prevent Layout Thrashing.
    const wrapperTops = wrappersRef.current.map(w => w ? getElementOffset(w) : 0);
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (hasChanged) {
        // Use fixed precision only for the CSS string output, not the underlying math
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // When using window scroll, just listen to scroll events
      // The global Lenis instance in App.jsx handles smooth scrolling
      const onScroll = () => handleScroll();
      window.addEventListener('scroll', onScroll, { passive: true });

      // Also run on rAF for smoother updates with Lenis
      const raf = () => {
        handleScroll();
      ...
      // Store cleanup ref
      lenisRef.current = { _windowListener: onScroll };
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      if (lenisRef.current) {
        if (lenisRef.current._windowListener) {
          window.removeEventListener('scroll', lenisRef.current._windowListener);
        } else if (lenisRef.current.destroy) {
